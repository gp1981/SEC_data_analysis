...
To recreate a balance sheet from the data in `df_Facts` of Apple Inc., we need to identify the relevant concepts and then associate the values and corresponding date.

(work in progress ...)
```{r summarize_Facts, message=FALSE, warning=FALSE, message=FALSE}
# Create df_Facts_summary with unique combinations of label and description
df_Facts_summary <- df_Facts %>%
  select(label, description) %>%
  distinct()

# Task 1: Build a new dataframe with labels from "standardized_balancesheet_label" ---------------------------------------------------------------------
...
# Create a new dataframe with labels from standardized_balancesheet_label
df_new <- data.frame(standardized_balancesheet_label = standardized_balancesheet$standardized_balancesheet_label)

# Task 2: Retrieve the value from df_Facts associated with the label for the latest period ---------------------------------------------------------------------
# Convert 'end' column to date format
df_Facts$end <- as.Date(df_Facts$end)

# Filter df_Facts to get values for the latest period for each label
df_latest_values <- df_Facts %>%
  group_by(label) %>%
  filter(end == max(end)) %>%
  select(label, val) %>%
  rename(df_Facts_value = val)

# Change the join column to match the column name in df_new
df_new <- left_join(df_new, df_latest_values, by = c("standardized_balancesheet_label" = "label"))
...
The objective is to generate standardized financial reports of Balancesheet, Income Statement, Cash Flow that so that we can use the same label to perform specific calculation across all companies.
