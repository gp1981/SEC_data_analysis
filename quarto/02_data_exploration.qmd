...
To recreate a balance sheet from the data in `df_Facts`, we need to identify the relevant concepts and then associate the values and corresponding date. 

First we start building our own standardized balancesheet:
...
Next step is to match the label in `df_Facts` with our standardized balancehseeet. There are different ways to identify the appropriate label. For instance we could use sophisiticated machine learning to read and interpret the long description of the labels matching with the 
